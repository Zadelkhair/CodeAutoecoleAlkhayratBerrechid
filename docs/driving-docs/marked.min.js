...
`)
                ...
                } return { type: "blockquote", raw: l, tokens: o, text: a }
            }
        } list(g) {
            let l = this.rules.block.list.exec(g); if (l) {
                let e = l[1].trim(), t = 1 < e.length, n = { type: "list", raw: "", ordered: t, start: t ? +e.slice(0, -1) : "", loose: !1, items: [] }, s = (e = t ? "\\d{1,9}\\" + e.slice(-1) : "\\" + e, this.options.pedantic && (e = t ? e : "[*+-]"), this.rules.other.listItemRegex(e)), i = !1; for (; g;) {
                    let e = !1, a = "", o = ""; if (!(l = s.exec(g)) || this.rules.block.hr.test(g)) break; a = l[0], g = g.substring(a.length); let h = l[2].split(`
...
`, g = g.substring(c.length + 1), e = !0), !e) {
                        let r = this.rules.other.nextBulletRegex(u), n = this.rules.other.hrRegex(u), s = this.rules.other.fencesBeginRegex(u), i = this.rules.other.headingBeginRegex(u), l = this.rules.other.htmlBeginRegex(u); for (; g;) {
                            let e = g.split(`
...
`+ t.slice(u); else {
                                if (p || 4 <= h.replace(this.rules.other.tabCharGlobal, "    ").search(this.rules.other.nonSpaceChar) || s.test(h) || i.test(h) || n.test(h)) break; o += `
`+ c
                            } p || c.trim() || (p = !0), a += e + `
`, g = g.substring(e.length + 1), h = t.slice(u)
                        }
                    } n.loose || (i ? n.loose = !0 : this.rules.other.doubleBlankLine.test(a) && (i = !0)); let t = null, r; this.options.gfm && ((t = this.rules.other.listIsTask.exec(o)) && (r = "[ ] " !== t[0], o = o.replace(this.rules.other.listReplaceTask, ""))), n.items.push({ type: "list_item", raw: a, task: !!t, checked: r, loose: !1, text: o, tokens: [] }), n.raw += a
                } let r = n.items.at(-1); if (r) { r.raw = r.raw.trimEnd(), r.text = r.text.trimEnd(), n.raw = n.raw.trimEnd(); for (let r = 0; r < n.items.length; r++)if (this.lexer.state.top = !1, n.items[r].tokens = this.lexer.blockTokens(n.items[r].text, []), !n.loose) { let e = n.items[r].tokens.filter(e => "space" === e.type), t = 0 < e.length && e.some(e => this.rules.other.anyLine.test(e.raw)); n.loose = t } if (n.loose) for (let e = 0; e < n.items.length; e++)n.items[e].loose = !0; return n }
            }
        } html(e) { e = this.rules.block.html.exec(e); if (e) return { type: "html", block: !0, raw: e[0], pre: "pre" === e[1] || "script" === e[1] || "style" === e[1], text: e[0] } } def(e) { let t = this.rules.block.def.exec(e); var r, n; if (t) return e = t[1].toLowerCase().replace(this.rules.other.multipleSpaceGlobal, " "), r = t[2] ? t[2].replace(this.rules.other.hrefBrackets, "$1").replace(this.rules.inline.anyPunctuation, "$1") : "", n = t[3] && t[3].substring(1, t[3].length - 1).replace(this.rules.inline.anyPunctuation, "$1"), { type: "def", tag: e, raw: t[0], href: r, title: n } } table(e) {
            let s = this.rules.block.table.exec(e); if (s && this.rules.other.tableDelimiter.test(s[2])) {
                let t = U(s[1]), e = s[2].replace(this.rules.other.tableAlignChars, "").split("|"), r = s[3]?.trim() ? s[3].replace(this.rules.other.tableRowBlankLine, "").split(`
`) : [], n = { type: "table", raw: s[0], header: [], align: [], rows: [] }; if (t.length === e.length) { for (var i of e) this.rules.other.tableAlignRight.test(i) ? n.align.push("right") : this.rules.other.tableAlignCenter.test(i) ? n.align.push("center") : this.rules.other.tableAlignLeft.test(i) ? n.align.push("left") : n.align.push(null); for (let e = 0; e < t.length; e++)n.header.push({ text: t[e], tokens: this.lexer.inline(t[e]), header: !0, align: n.align[e] }); for (var l of r) n.rows.push(U(l, n.header.length).map((e, t) => ({ text: e, tokens: this.lexer.inline(e), header: !1, align: n.align[t] }))); return n }
            }
...
`} hr(e) {
            return `<hr>
`} list(t) {
            let e = t.ordered, r = t.start, n = ""; for (let e = 0; e < t.items.length; e++) { var s = t.items[e]; n += this.listitem(s) } var i = e ? "ol" : "ul"; return "<" + i + (e && 1 !== r ? ' start="' + r + '"' : "") + `>
`+ n + "</" + i + `>
`} listitem(e) {
...
`} table(t) {
            let e = "", r = ""; for (let e = 0; e < t.header.length; e++)r += this.tablecell(t.header[e]); e += this.tablerow({ text: r }); let n = ""; for (let e = 0; e < t.rows.length; e++) { var s = t.rows[e]; r = ""; for (let e = 0; e < s.length; e++)r += this.tablecell(s[e]); n += this.tablerow({ text: r }) } return `<table>
<thead>
`+ e + `</thead>
